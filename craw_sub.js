...
async function fetchFromPexels(keyword, type, targetDir, neededCount) {
    ...
    try {
        ...
        for (const item of (results || [])) {
            if (downloaded >= neededCount) break;
            let downloadUrl = type === 'video' ? (item.video_files.find(v => v.quality === 'hd') || item.video_files[0])?.link : item.src.large;
            if (downloadUrl && await downloadFileHelper(downloadUrl, targetDir, type === 'video' ? 'mp4' : 'jpg')) downloaded++;
        }
...
async function fetchFromPixabay(keyword, type, targetDir, neededCount) {
    ...
    let downloaded = 0;
    const url = `https://pixabay.com/api/${type === 'video' ? 'videos' : ''}/?key=${PIXABAY_API_KEY}&q=${encodeURIComponent(keyword)}&per_page=${Math.max(3, neededCount * 2)}&safesearch=true`;
    try {
        const response = await fetch(url);
        if (!response.ok) return 0;
        const data = await response.json();
...
// ==========================================
// 4. HỆ THỐNG QUẢN LÝ HÀNG ĐỢI (SERIAL QUEUE MANAGER)
// ==========================================
