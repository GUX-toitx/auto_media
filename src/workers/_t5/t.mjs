...
globalThis.__mode='ok'; globalThis.__calls=[];
function httpsPost(url,headers,body){globalThis.__calls.push(body);
 const wrap=(p)=>Promise.resolve({status:200,body:JSON.stringify({status:'completed',usage:{},output:[{type:'message',content:[{type:'output_text',text:JSON.stringify(p)}]}]})});
 if(body?.text?.format?.name==='srt_translation'){
  const n=(body.input.match(/Return EXACTLY (\d+) strings/)||[])[1]|0;
  const lines=body.input.split('===== SUBTITLE LINES =====')[1].trim().split('\n');
  if(globalThis.__mode==='lech' && n>1) return wrap({translations:lines.slice(0,n-1).map(l=>'JA:'+l.replace(/^\d+\. /,''))});
  if(globalThis.__mode==='hong') return Promise.resolve({status:500,body:'boom'});
  return wrap({translations:lines.map(l=>'JA:'+l.replace(/^\d+\. /,''))});}
 return Promise.resolve({status:500,body:'khong mong doi'});}
...
// Dịch cả file. Lô nào lệch số dòng thì thử lại 1 lần, vẫn lệch thì CHIA ĐÔI lô rồi dịch tiếp
// (lô nhỏ dễ khớp hơn); tới đơn lẻ mà vẫn hỏng thì giữ nguyên câu gốc cho dòng đó — mất bản dịch
// 1 dòng vẫn hơn là lệch toàn bộ phần còn lại của file.
async function translateSrtTexts(texts, targetLang, projectId) {
    ...
    const run = async (start, end, depth) => {
        ...
        for (let attempt = 1; attempt <= 2; attempt++) {
            try {
                const got = await translateChunk(lines, targetLang, glossary.text, tag);
                if (got) {
            ...
            } catch (e) { console.warn(`[dịch SRT] ⚠️ lô ${tag} lỗi (lần ${attempt}/2): ${e.message}`); }
        }
        ...
        for (let i = start; i < end; i++) { out[i] = texts[i]; failed++; }   // bó tay -> giữ câu gốc
        console.warn(`[dịch SRT] ❌ giữ nguyên câu gốc cho dòng ${start + 1}-${end}`);
    };
...
async function main() {
    ...
    if (isPromptMode) {
        ...
        if (wantTranslate && !translatedSentences && targetLang !== 'vi') {
            ...
            const translated = await translateSrtTexts(srtSentences.map(s => s.text), targetLang, projectId);
            const outPath = path.join(MEDIA_DIR, projectId, 'input_translated.srt');
            fs.mkdirSync(path.dirname(outPath), { recursive: true });
            fs.writeFileSync(outPath, buildSrtFile(srtSentences, translated));
            console.log(`[sports_srt] 💾 Đã ghi file SRT đã dịch: ${outPath}`);
            translatedSentences = srtSentences.map((s, i) => ({ ...s, text: translated[i] }));
...
export { translateSrtTexts, buildSrtFile, parseSrt, tcToSec };
