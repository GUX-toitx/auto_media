...
import { initDB } from '../lib/migrate.js';
import { fetchFromDvidsBot } from '../crawlers/dvidsCrawler.js';
import { fetchFromBellingcatBot } from '../crawlers/bellingcatCrawler.js';
import { fetchFromApnewsBot } from '../crawlers/apnewsCrawler.js';
import { fetchFromAlJazeeraBot } from '../crawlers/aljazeeraCrawler.js';
import { fetchFromCnnBot } from '../crawlers/cnnCrawler.js';
...
async function processNextInQueue() {
    ...
    try {
        ...
        if (crawlType === 'Video ID') {
            ...
            execSync(`yt-dlp --cookies ./config/youtube.com_cookies.txt --match-filter "duration <= 300" -f "bestvideo[ext=mp4][height<=1080]+bestaudio[ext=m4a]/mp4" -o "${targetDir}/original.%(ext)s" "${projectId}"`, { stdio: 'inherit' });
            
            // Lệnh lấy Subtitle giữ nguyên
            execSync(`yt-dlp --cookies ./config/youtube.com_cookies.txt --write-sub --write-auto-subs --sub-lang ${lang} --convert-subs srt --skip-download -o "${targetDir}/original.%(ext)s" "${projectId}" --ignore-errors`, { stdio: 'inherit' });
