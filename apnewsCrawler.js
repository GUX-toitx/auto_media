    ...
    try {
        ...
        for (const link of articleLinks) {
            ...
            try {
                ...
                if (type === 'video') {
                     // AP News ít dùng video mp4 trực tiếp, thường nhúng player. Ta cứ quét dự phòng thẻ meta
                    const ogVideo = $$('meta[property="og:video"]').attr('content');
                    if (ogVideo && ogVideo.endsWith('.mp4')) mediaUrls.push(ogVideo);
                } else {
                    // Ưu tiên 1: Ảnh đại diện bài viết
                    const ogImage = $$('meta[property="og:image"]').attr('content');
                    if (ogImage) {
                         // AP News đôi khi có tham số kích thước ở URL ảnh, ta cố gắng lấy ảnh gốc
                         mediaUrls.push(ogImage);
                    }

                    // Ưu tiên 2: Ảnh trong bài viết
                    $$('picture source').each((i, el) => {
                        const srcset = $$(el).attr('srcset');
                        if (srcset) {
                            // Lấy link đầu tiên trong srcset (thường là link lớn nhất)
