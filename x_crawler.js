...
//
// Dùng như module: import { crawlX } from './x_crawler.js'
...
function readCookies(profileName) {
    const out = execFileSync('node', ['x_cookies.js', profileName], { encoding: 'utf8' });
    const j = JSON.parse(out);
    return `auth_token=${j.auth_token}; ct0=${j.ct0}`;
}

function runScrape(cookies, { keywords = '', urls = '', limit = 15, db = 'accounts.db' }) {
    const args = ['x_scrape.py', '--cookies', cookies, '--limit', String(limit), '--db', db];
...
function capture(profileName, url, outDir, base) {
    const r = spawnSync('node', ['x_capture.js', '--profile', profileName, '--url', url, '--out', outDir, '--name', base],
